...
class StrongCorePropagate(Propagate):
    """
    ...
    def execute(self, nodenet, nodes, netapi):
        ...
        for uid, node in nodes.items():
            for gate_type in node.get_gate_types():
                # print("node: " + node.name + " gatetype: " + gate_type)
                gate = node.get_gate(gate_type)
                for link in gate.get_links():
                    # print("link: ")
                    for sheaf in gate.sheaves:
                        ...
                        if targetsheaf in link.target_slot.sheaves:
                            # print(link.target_node.name + " " + link.target_slot.type + " " +  str(link.target_slot.sheaves[targetsheaf]['activation']) )
...
class StrongCoreCalculate(Calculate):
    """
...
class StrongCorePORRETDecay(StepOperator):
    """
